...
function edit_event() {
    ...
    window.location.href = "/eventSettings.html";
}
...
function get_link() {
    ...
    window.location.href = "rsvp.html?link=" + links[index];
}
    ...
    for (event in events) {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                xhttp.setRequestHeader("Content-type", "application/json");
                xhttp.send(JSON.stringify({ name: name, date: date, time: time }));
            }

        };
        xhttp.open("POST", "/checkCalendarEvent", true);
        xhttp.setRequestHeader("Content-type", "application/json");
        xhttp.send(JSON.stringify({ name: name, date: date, time: time }));
    }
